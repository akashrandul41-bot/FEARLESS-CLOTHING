...
function addToCartWithSize() {
  ...
  if (existingItem) {
    ...
    cart.push({ ...product, cartItemId, size: window.selectedSize, quantity: 1 });
  }

  saveCart();
  updateCartCount();
  alert(`${product.name} (Size: ${window.selectedSize}) added to cart!`);
}
...
function renderCart() {
  ...
  cart.forEach((item, index) => {
    ...
    itemEl.innerHTML = `
      ...
      <div class="cart-item-details">
        ...
        <p class="cart-item-price">$${item.price.toFixed(2)} ${item.size ? '<span style="color: var(--text-color); margin-left: 10px; font-weight: bold;">Size: ' + item.size + '</span>' : ''}</p>
      </div>
  ...
  if (shippingEl) shippingEl.textContent = shipping > 0 ? '$10.00' : '$0.00';
...
function processCheckout() {
  ...
  if (method === 'credit-card') {
    // This is where you initialize Stripe with your keys
    // You provided the Stripe Account ID (acct_1TVPFO14iE2Rk0CD).
    // You also need the Publishable Key (pk_live_...) from your dashboard.

    // Example:
    // const stripe = Stripe('pk_test_51TVPFhP4Hbs8kGpwx5nXmSjhW8MAeczqGVBMwLhFqXqT3uQLBm25e7FHrEo1itIGEMwINRZpzg5MJPcd2qQR6CsA00jBDat7vy', { stripeAccount: 'acct_1TVPFO14iE2Rk0CD' });

    alert("Stripe Integration Ready for Account: acct_1TVPFO14iE2Rk0CD \n\nTo make this work live, replace 'YOUR_PUBLISHABLE_KEY' in script.js with your actual Stripe Publishable Key (starts with pk_...).");
    ...
    setTimeout(() => {
      ...
      if (checkoutBtn) checkoutBtn.textContent = 'Complete Purchase';
    }, 2000);

  } else {
    alert(`Processing payment using ${method.toUpperCase()}...`);
    if (checkoutBtn) {
